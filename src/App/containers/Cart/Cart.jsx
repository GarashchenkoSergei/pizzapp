...
import { groupedCart } from './store/selectors';
...
class Cart extends Component {
  render() {
    const { cart, groupedCart, addToCart, removeFromCart, removeAllFromCart } = this.props;
            ...
            <ul className={styles.cart_list}>
              {groupedCart.map((item, index) => (
                <li key={index} className={styles.cart_item}>
                  <button className={styles.close_button} onClick={() => removeAllFromCart(item[0].id)}>X</button>
                  ...
                  <div className={styles.item_amount}>
                    <button className={styles.item_amount__change} onClick={() => removeFromCart(item[0].id)}>-</button>
                    <span className={styles.item_amount__number}>{item.length}</span>
                    <button className={styles.item_amount__change} onClick={() => addToCart(item[0])}>+</button>
                  </div>
